    ...
    public partial class FrmListadoVentas : Form
    {
            ...
            try
            {
                ...
                foreach (DataRow venta in dt.Rows)
                {
                    ...
                    dataLista.Rows.Add(venta["idventa"], venta["razon_social"], venta["fecha"], venta["tipo_comprobante"], venta["total"], estado, venta ["caja"], venta ["idcliente"],
                        ...
                        venta["CAE_Fechavencimiento"], venta["enstock"], venta ["formadepago"], venta ["PAGO"], venta["Multipago"] == "" ? "0" : venta["Multipago"]);
                }
        ...
        private void dataLista_CellMouseDoubleClick_1(object sender, DataGridViewCellMouseEventArgs e)
        {
               if (dataLista.Rows.Count > 0)
               {
                    ...
                    Convert.ToString(this.dataLista.CurrentRow.Cells["pago"].Value),
                    Convert.ToString(this.dataLista.CurrentRow.Cells["Multipago"].Value));
                    venta.ShowDialog();
                    this.buscarPorFecha();
                    this.actualizarTotal();
               }
        ...
        private void dataLista_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                if (Convert.ToString(row.Cells["Multipago"].Value) != "0")
                {
                    if (Convert.ToString(row.Cells["Multipago"].Value) != "")
                    {
                        return;
                    }
        ...
        private void MenuAnular_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (NegocioConfigEmpresa.usuariosa == true || objusuario.TieneRegla("47") == true)
                {
                    ...
                    NegocioVenta.anular(Convert.ToInt32(row.Cells["Codigo"].Value), Convert.ToBoolean(row.Cells["stock"].Value), 'A', Convert.ToInt32(row.Cells["Formadepago"].Value),concaja);

                    if (concaja && Convert.ToInt32(row.Cells["Formadepago"].Value) != 5)
                    {
