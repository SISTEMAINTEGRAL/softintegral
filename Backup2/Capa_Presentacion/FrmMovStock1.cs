...
namespace Capa_Presentacion
{
    public partial class FrmMovStock1 : Form
    {
        ...
        {
            if (txtProveedor.TextLength >= 2 && IsNumeric(txtProveedor.Text) != true)
            {
                ...
                Dtproveedor.Columns[1].Width = 350;
                Dtproveedor.Columns[2].Visible = false;
        ...
        private void dataLista_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right && dataLista.CurrentRow.Cells["estado"].Value.ToString() == "EMITIDO")
            {
                // DGVenta.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
                // mouse = true;
                contextMenuStrip2.Show(dataLista, new Point(e.X, e.Y));
        ...
        {
            DataTable datadetallemovstock = new DataTable();
            string ingresouegreso = "INGRESO";

            datadetallemovstock =  NegocioMovStock.mostrarDetalleMovStock(Convert.ToInt32(dataLista.CurrentRow.Cells["idmov_stock"].Value));
            if (dataLista.CurrentRow.Cells["movimiento"].Value == "INGRESO")
            {
                ingresouegreso = "EGRESO";
            }

            NegocioMovStock.anular(Convert.ToInt32(dataLista.CurrentRow.Cells["idmov_stock"].Value), ingresouegreso);
            dataLista.DataSource = NegocioMovStock.buscarFecha(dtpFechaIni.Text, dtpFechaFin.Text, ChkProveedor.Checked == true ? Convert.ToInt32(LblIdProveedor.Text) : 0);
