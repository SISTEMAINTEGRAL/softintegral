...
namespace Capa_Presentacion
{
    public partial class frmPventa2 : Form
    {
        ...
        private long idturno = 0;
        private decimal limitectacte = 0;
        private bool habilitarctacte = false;
        private decimal saldodeudorctacte = 0;
        ...
        private void frmPventa_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //}
                int indexcategoria;
                if (NegocioConfigEmpresa.confsistema("tipoventainicial").ToString() == "B")
                {
                    indexcategoria = 1;
                }
                else
                ...
                if (NegocioConfigEmpresa.emciva == "MN")
                {
                    ...
                    cbxCategoria.Text = "C";
                    if (NegocioConfigEmpresa.confsistema("tipoventainicial").ToString() == "C")
                    {
                        indexcategoria = 0;
                    }
                    else
               ...
               // this.DGVenta.Columns["Precio"].DefaultCellStyle.Format = String.Format("$###,##0.00");
                ...
                cbxCategoria.SelectedIndex = indexcategoria;
        ...
        public frmPventa2()
        {
          ...
          public void Buscar_Cliente(string codCliente, bool porcodigo = false)
        {
            try
            {
                if (codCliente != "")
                {
                    DataTable dt;
                    if (NegocioConfigEmpresa.confsistema("codigointernocliente").ToString() == "True" && codCliente != "1" && porcodigo == false)
                    {
                    ...
                    if (dt.Rows.Count != 0)
                    {
                        ...
                        limitectacte =Convert.ToDecimal( row["limitedectacte"].ToString());
                        habilitarctacte = Convert.ToBoolean(row["habilitarctacte"].ToString());
                        saldodeudorctacte = Convert.ToDecimal(row["saldo"].ToString());
          ...
          //  Frmimpventicket miform = new Frmimpventicket();
           ...
           // miform.Show();
            ...
            try
            {
                ...
                {
                    Buscar_Cliente(cliente.IdCliente.ToString(),true);
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            else if (rctacte.Checked == true) 
            {
                ...
                formadepago = "ctacte";
                string mensajectacte = "";
                mensajectacte = chequearctacte(habilitarctacte, limitectacte, saldodeudorctacte + Convert.ToDecimal(txtTotalPagar.Text));
                
                if (mensajectacte != "ok")
                {
                    UtilityFrm.mensajeError(mensajectacte);
                    return;
                }
            }
            ...
            FrmGuardarVenta2 venta = new FrmGuardarVenta2(decimal.Round(Convert.ToDecimal(txtTotalPagar.Text), 2), Convert.ToInt32(idcliente), decimal.Round(Convert.ToDecimal(txtIVA.Text), 2), decimal.Round(Convert.ToDecimal(txtNeto.Text), 2), formadepago, nombretarjeta, importecuota, cuota, Convert.ToInt32(cbTarjeta.SelectedValue),cuit,txtRazonSocial.Text ,cbxCategoria.Text ,riva,domicilio,Convert.ToDecimal (txtIva105.Text),Convert.ToDecimal( txtNeto105.Text),pendientedestock);
            ...
            actualizarultimoidcierreidturno();
            objcaja.extestadoturno(1, "", NegocioConfigEmpresa.nrocaja, idcierre,idturno);

             if (objcaja.Estadocierre == "CERRADO" && venta.Concaja == true)
             {
                venta.Concaja = false;
                mensaje = "No hay ningun turno abierto, abrir el turno para poder continuar";
             }
        ...
        public void actualizarultimoidcierreidturno()
        {
            Negociocaja objcaja = new Negociocaja();
            ...
            objcaja.extraercierre("id_turno", NegocioConfigEmpresa.nrocaja, idcierre);
        ...
        private string chequearctacte(bool habilitarctacte, decimal limitectacte, decimal saldodeudor)
        {
            string msg = "ok";
            if (habilitarctacte == false)
            {
                msg = "No se le puede vender a ctacte a este cliente, no esta habilitado para generar ctacte";
            }
            if (limitectacte > 0)
            {
                if (saldodeudor > limitectacte)
                {
                    msg = "No se le puede vender a ctacte a este cliente, el limite es de " + limitectacte + " y su saldo deudar es de " + saldodeudor;
                    return msg;
                }
            }
            return msg; 
        ...
        private void btnProducto_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (!promo(objconsulta.Idarticulo, "poridarticulo","", 1,ref index))
                    {
        ...
        private int recorrerDGventaeliminaridarticulosinpromo(long idarticulo, bool eliminar = false)
        {
            encontrado = false;
            int cont = 0;
            foreach (DataGridViewRow row in DGVenta.Rows)
        ...
        private bool promo(long idarticulo,string tipo, string codigobarra, decimal cantidad, ref int indice)
        {
            ...
            if (objnart.Sindatos == true)
            {
                cantidad = cantidad + recorrerDGventaeliminaridarticulosinpromo(objnart.IdArticulo);
                if (objnart.Preciopormayor2 > 0 && objnart.Cantidadpormayor2 > 0 && cantidad >= objnart.Cantidadpormayor2)
                {
                    ...
                    if (promocombo(cantidad, ref cantidadpromo, ref cantidadsinpromo))
                    {
                        ...
                        recorrerDGventaeliminaridarticulosinpromo(objnart.IdArticulo, true);

                        if (cantidadpromo > 0)
                        {
                            indice = Buscar_producto(objnart.IdArticulo, "poridarticulo", false, cantidadpromo, "", "conpromo");
                        }

                        if (cantidadsinpromo > 0)
                        {

                            indice = Buscar_producto(objnart.IdArticulo, "poridarticulo", false, cantidadsinpromo, "", "sinpromo");
        ...
        private void DGCliente_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                    ...
                    Buscar_Cliente(DGCliente.CurrentRow.Cells["idcliente"].Value.ToString(),true);
                    DGCliente.Visible = false;
                    txtNombreProducto.Focus();
        ...
        private void DGVenta_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
