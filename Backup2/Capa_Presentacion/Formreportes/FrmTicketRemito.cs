...
namespace Capa_Presentacion.Formreportes
{
    public partial class FrmTicketRemito : Form
    {
        ...
        private void imprimir()
        {
            ...
            string formatoimpresionremito = "A4";
            string impresora = "";

            midatareader = NegocioConfigEmpresa.mostrarformatoimpresora(3);

            if (midatareader.Read() == true)
            {
                impresora = Convert.ToString(midatareader["impresora"]);
                formatoimpresionremito = Convert.ToString(midatareader["formato"]);
            }
