...
namespace Capa_Presentacion
{
    public partial class FrmCategoria : Form
    {
       ...
       private bool isNuevo, isEditar;
        private string opcioncategoria = "CATEGORIA";
       ...
       //comparo el evento pasado como parametro y el enum keys.enter
        ...
        private void dataLista_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right )
            {
                ...
                // mouse = true;
                opcioncategoria = "CATEGORIA";
                contextMenuStrip1.Show(dataLista, new Point(e.X, e.Y));

            }
        }

        private void DGSubcategoria_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                // DGVenta.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
                // mouse = true;
                opcioncategoria = "SUBCATEGORIA";
                contextMenuStrip1.Show(DGSubcategoria, new Point(e.X, e.Y));
        ...
        {
            if (opcioncategoria == "CATEGORIA")
            {
                NegocioCategoria.eliminar(Convert.ToInt32(dataLista.CurrentRow.Cells["idcategoria"].Value),0,opcioncategoria);
            }
            else
            {
                NegocioCategoria.eliminar(Convert.ToInt32(DGSubcategoria.CurrentRow.Cells["idcategoria"].Value), Convert.ToInt32(DGSubcategoria.CurrentRow.Cells["idsubcategoria"].Value), opcioncategoria);
            }
            mostrar();
            DGSubcategoria.DataSource = NegocioCategoria.buscar("", Convert.ToInt32(dataLista.CurrentRow.Cells["idcategoria"].Value));
        }
